...
describe('NexusNode', () => {
    test('init', () => {
        const n = new NexusNode('idea',10,20,'T');
        expect(n.type).toBe('idea'); expect(n.x).toBe(10); expect(n.label).toBe('T');
        expect(n.content).toBe(''); expect(n.notes).toBe(''); expect(n.properties).toEqual({});
    });
    test('default label', () => { expect(new NexusNode('topic',0,0).label).toBe('Topic'); });
});
...
describe('WorldModel', () => {
    ...
    test('addNode', () => { const n=m.addNode('idea',1,2,'X'); expect(m.nodes.get(n.id).label).toBe('X'); });
    test('removeNode cascades', () => {
        const a=m.addNode('idea',0,0),b=m.addNode('topic',1,1);
...
describe('Serialization', () => {
    test('round-trip', () => {
        ...
        const a=m.addNode('rule',10,20,'Law'); a.content='[[x]]'; a.properties.k='v';
        const b=m.addNode('event',30,40,'Trig'); m.addEdge(a.id,b.id,'causes');
    ...
    test('missing fields', () => {
        const m=new WorldModel();
        m.fromJSON({nodes:[{id:'n1',type:'idea',x:0,y:0,label:'T'}],edges:[]});
...
describe('Search', () => {
    ...
    beforeEach(() => {
        m=new WorldModel();
        const a=m.addNode('idea',0,0,'Quantum'); a.description='small'; a.content='[[uncertainty]]';
        const b=m.addNode('topic',1,1,'Relativity'); b.notes='Einstein';
        const c=m.addNode('note',2,2,'Grocery'); c.properties.cat='personal';
    });
...
describe('Edge cases', () => {
    ...
    test('1000 nodes', () => { for(let i=0;i<1000;i++)m.addNode('idea',i,i,`N${i}`); expect(m.nodes.size).toBe(1000); });
    test('special chars roundtrip', () => {
        const n=m.addNode('idea',0,0,'A"B"&<C>'); n.content='[[w]] ![[e]]';
        const m2=new WorldModel(); m2.fromJSON(m.toJSON());
        expect(m2.nodes.get(n.id).content).toBe('[[w]] ![[e]]');
    });
    test('self-edge cleanup', () => {
        const a=m.addNode('idea',0,0); m.addEdge(a.id,a.id);
...
describe('Debate subdrawer', () => {
    test('filters debate children', () => {
        const m=new WorldModel();
        const t=m.addNode('idea',0,0,'Topic');
        const a=m.addNode('idea',0,100); a.properties={side:'A',round:'1'};
        const b=m.addNode('idea',0,200); b.properties={side:'B',round:'1'};
        const r=m.addNode('rule',0,300); r.properties={type:'resolution'};
        const x=m.addNode('note',5,5);
